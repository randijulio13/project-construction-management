...
export function middleware(request: NextRequest) {
    ...
    const isProtectedRoute = internalPathname.startsWith("/dashboard") || 
                           internalPathname.startsWith("/warehouse") || 
                           internalPathname.startsWith("/projects") ||
                           internalPathname.startsWith("/account-settings");
    const isAuthRoute = internalPathname === "/login" || internalPathname === "/register";
...
export const config = {
    ...
    matcher: ['/', '/(id|en)/:path*', '/((?!api|_next|_static|_vercel|[\\w-]+\\.\\w+).*)']
};
